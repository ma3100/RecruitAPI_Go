...
type resultGourmetResponse struct {
	Results struct {
...
func GetGourmet(apiKey string, format string, parameterString string) resultGourmetResponse {
	url := "http://webservice.recruit.co.jp/hotpepper/gourmet/v1/?key=" + apiKey + "&format=" + format + "&" + parameterString

	byteArray := commonLogic.ExecuteGetRequest(url)
	fmt.Println(string(byteArray))

	data := resultGourmetResponse{}
